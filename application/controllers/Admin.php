...
//登录
    public function check_login()
    {
        ...
        $md5psw = md5($password);
        $row = $this->admin_model->get_name_pwd($adminName, $md5psw);
        if ($row) {
            $adminId = -1;
    ...
    public function admin_mgr()
    {
        ...
        $order_col = array('0' => 'admin_id', '1' => 'username', '2' => 'tel', '3' => 'level', '4' => 'open_id');
...
//        $img = 'uploads/head/' . $upload_data['raw_name'] . $upload_data['file_ext'];
        $row = $this->admin_model->add_admin($name, md5($pwd), $lev,$real_name,$tel);
        if ($row) {
            redirect("admin/index");
        } else {
            echo "添加失败";
