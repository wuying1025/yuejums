...
class House extends CI_Controller
{
    ...
    public function house_add(){
        ...
        $structure_area = htmlspecialchars($this->input->post('structure_area'));
    ...
    public function house_edit(){
        ...
        $result = $this -> house_model -> get_house_img($house_id);
...
//        $arr['unorder'] = $unorder;
        ...
        $order_col = array('0' => 'house_id', '1' => 'title', '2'=>'plot_name' ,'3' => 'street', '4' => 'price');

        $recordsTotal = $this->house_model->get_total_count();
        $recordsFiltered = $this->house_model->get_filterd_count_list($search,$status);

        $datas = $this->house_model->get_paginated_houses_list($length, $start, $search,$status, $order_col[$order_col_no], $order_col_dir);
...
//
    ...
    public function house_recover(){
        $order_id = $this ->input ->get('houseId');
        $row =  $this->house_model->recover_house($order_id);
        if($row > 0){
            echo 'success';
        }else{
            echo 'fail';
