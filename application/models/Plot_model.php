...
//        $this->db->delete('t_plot',array('plot_id' => $plot_id));
    ...
    public function update_plot_name($id,$name,$deve,$plot_description,$plot_video,$plot_pos){
        $this->db->where_in('plot_id',$id);
        $this->db->update('t_plot', array('plot_name' => $name,'developer_id'=>$deve,'description'=>$plot_description,'video'=>$plot_video,'plot_pos'=>$plot_pos));
        return $this->db->affected_rows();
    }
}