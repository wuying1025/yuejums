...
//    //获取所有小区
    ...
    public function get_filterd_count_list($search,$status)
    {

        $sql = "SELECT * FROM t_house JOIN t_plot on t_plot.plot_id = t_house.plot_id WHERE t_house.is_delete=0 ";
        if($status == 1){
            $sql = "SELECT * FROM t_house JOIN t_plot on t_plot.plot_id = t_house.plot_id WHERE t_house.is_delete=0 and t_house.house_id not in (select house_id from t_order where t_order.status = '入住中') ";
        }else if($status == 2){
            $sql = "SELECT * FROM t_house JOIN t_plot on t_plot.plot_id = t_house.plot_id WHERE t_house.is_delete=0 and t_house.house_id in (select house_id from t_order where t_order.status = '入住中') ";
        }else if($status == 3){
            $sql = "SELECT * FROM t_house JOIN t_plot on t_plot.plot_id = t_house.plot_id left JOIN t_order on t_order.house_id = t_house.house_id
                WHERE t_house.is_delete=0 and t_order.start_time >= curdate()";
        }
        if (strlen($search) > 0) {
            $sql .= " and (t_house.title LIKE '%" . $search . "%' or t_house.street LIKE '%" . $search . "%' or t_house.price LIKE '%" . $search . "%' or  t_plot.plot_name LIKE '%" . $search . "%')";
        }
        return $this->db->query($sql)->num_rows();
    }

    public function get_house_img($house_id){
        return $this->db->get_where('t_house_img',array('house_id'=>$house_id,'is_main'=>1))->result();
    }

    public function get_paginated_houses_list($limit, $offset, $search, $status,$order_col, $order_col_dir)
    {
        ...
        $sql .= " ORDER BY t_house.$order_col $order_col_dir";
        $sql .= " LIMIT $offset, $limit";
        return $this->db->query($sql)->result();
    }
    ...
    public function recover_house($house_id){
        $this->db->where('house_id', $house_id);
        $this->db->update('t_house', array('is_delete' => 0));
        return $this->db->affected_rows();
    }
}