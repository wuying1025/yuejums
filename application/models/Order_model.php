...
//            $result=$this->db->delete('t_facility_type', array('type_id' => $namearr[$key]));
    ...
    {
        $sql = "select * from t_order where house_id = " . $houseId . " and ((unix_timestamp('" . $startTime . "')>=unix_timestamp(start_time) and unix_timestamp('" . $startTime . "') < unix_timestamp(end_time) and (status = '已支付' or status = '进行中' or status = '未支付')) or (unix_timestamp('" . $endTime . "') > unix_timestamp(start_time) and unix_timestamp('" . $endTime . "') <= unix_timestamp(end_time) and (status = '已支付' or status = '进行中' or status = '未支付')) or (unix_timestamp('" . $startTime . "') <= unix_timestamp(start_time) and unix_timestamp('" . $endTime . "') >= unix_timestamp(end_time) and (status = '已支付' or status = '进行中' or status = '未支付')))";

        return $this->db->query($sql)->result();
    }

    public function is_free_house_unorder($houseId, $startTime, $endTime,$order_id)
    {
        $sql = "select * from t_order where order_id != ". $order_id ." and house_id = " . $houseId . " and ((unix_timestamp('" . $startTime . "')>=unix_timestamp(start_time) and unix_timestamp('" . $startTime . "') < unix_timestamp(end_time) and (status = '已支付' or status = '进行中' or status = '未支付')) or (unix_timestamp('" . $endTime . "') > unix_timestamp(start_time) and unix_timestamp('" . $endTime . "') <= unix_timestamp(end_time) and (status = '已支付' or status = '进行中' or status = '未支付')) or (unix_timestamp('" . $startTime . "') <= unix_timestamp(start_time) and unix_timestamp('" . $endTime . "') >= unix_timestamp(end_time) and (status = '已支付' or status = '进行中' or status = '未支付')))";
    ...
    public function get_date_by_house($house_id)
    {
        ...
        $this->db->from('t_order');
        $this->db->where('house_id',$house_id);
        $this->db->where_in('status', ["入住中","已支付"]);
        return $this->db->get()->result();
    }
    ...
    public function get_checkin_user($orderId){
        return $this->db->get_where("t_checkin",array('order_id'=>$orderId))->result();
    }
    public function del_check($orderId){
        $this->db->delete('t_checkin', array('order_id' => $orderId));
    }
    ...
    public function add_enter($data){
        $this -> db -> insert_batch('t_checkin',$data);
        return $this -> db -> affected_rows();
    }
}