...
<script id="order-edit-tpl" type="text/html">
    <?php include 'tpls/order_edit_tpl.html'; ?>
</script>
<!--订单添加-->
...
//                    var val = $(this.hash + '-table').parent().children('.reportrange span').html();
            ...
            "columnDefs": [
                ...
                {"orderable": false, "targets": [0,1,2,3,4,5,6,7,8]},
                {
                    ...
                    "data": null,
                    "render": function (data, type, row) {
                        return '<a href="javascript:;" class="btn-edit" data-id="' + row.order_id + '">编辑</a> | <a href="javascript:;" class="btn-del" data-id="' + row.order_id + '">删除</a>';
//                        return '';
                    },
                },
                {"className": "show-order-detail", "targets": [1, 2, 3, 4, 5, 6, 7, 8]}
            ], /*
             "dom":
...
//            initComplete: initComplete
        ...
        //编辑
        $('#example tbody').css({cursor: 'pointer'}).on('click', '.btn-edit', function (e) {
            var orderId = $(this).data('id');
            $.sidepanel({
                width: 700,
                title: '订单编辑',
                tpl: 'order-edit-tpl',
                dataSource: 'order/order_edit',
                data: {
                    orderId: orderId
                },
                callback: function () {
                    ...
                    var creatAdd = function () {
                        var $body = $('<div class="checkIn-container">' +
                            ...
                            '<label class="col-lg-2 col-sm-2 control-label">入住用户</label>' +
                            '<div class="col-lg-10">' +
                            '<input type="text" name="checkin_name" class="form-control checkin_name" placeholder="姓名">' +
                            '</div>' +
                            '</div>' +
                            '<div class="form-group">' +
                            '<label class="col-lg-2 col-sm-2 control-label">联系方式</label>' +
                            '<div class="col-lg-10">' +
                            '<input type="text" name="checkin_phone" class="form-control checkin_phone" placeholder="联系方式 ">' +
                            '</div>' +
                            '</div>' +
                            '<div class="form-group">' +
                            '<label class="col-lg-2 col-sm-2 control-label">身份证号</label>' +
                            '<div class="col-lg-10">' +
                            '<input type="text" name="checkin_card" class="form-control checkin_card" placeholder="证件号">' +
                            '</div>' +
                            '</div>' +
                            '<legend></legend>' +
                            '</div>' +
...
//                            $(".checkIn-container-dad .ee:last-child input").val("");
                    ...
                    $(".checkIn-container-dad").on("click",'.delete', function () {
                        console.log($(this).parents('.form-group'));
                        $(this).parents('.form-group').prev().remove();
                    ...
                    $('#edit-order').on('click', function () {
                        ...
                        var arrayPhone = [];
                        var arrayCard = [];
                        $(".checkin_name").each(function () {
                            console.log($(this).val());
                            var name = $(this).val();
                            arrayName.push(name);
                        });
                        $(".checkin_phone").each(function () {
                            var phone = $(this).val();
                            arrayPhone.push(phone);
                        });
                        $(".checkin_card").each(function () {
                            var card = $(this).val();
                            arrayCard.push(card);
                        });
                        var house_id = $(".house-choose-end").attr('data-id'),
                            ...
                            price = $('.price').val(),
                            urgency_concat = $('.urgency-concat').val(),
                            urgency_contact_tel = $('.urgency_contact_tel').val(),
                            cash_pledge = $('.cash_pledge').val(),
                            return_cash_pledge = $('.return_cash_pledge').val(),
                            cash_pledge_pay_way = $(':radio:checked').val(),
                            add_time = $('.add_time').val(),
                            order_type = $('.order_type').val(),
                            order_memo = $('.order_memo').val(),
                            checkout_mark = $('.checkout_mark').val(),
                            checkin_memo = $('.checkin_memo').val(),
                            checkout_has_problem = $('.checkout_has_problem').val(),
